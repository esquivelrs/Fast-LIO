...
- Ouster 128 config and launch file
- Ouster pointcloud ring field definition: from `uint8_t` to `uint16_t` in `src/preprocess.cpp`

### Instructions

Follow the Fast-LIO instruction in the original readme and source both the `ws` space and the `livox`

Check the topics in the configuration file and run:

```
ros2 launch fast_lio mapping_ouster.launch.py
```
...
Based on [reposiory](https://github.com/Taeyoung96/FAST_LIO_ROS2)
